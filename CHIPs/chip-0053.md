...
Continuing from the previous example, in order to distribute the payments, the top coin is spent. This creates 100 intermediate coins, each of which needs to be spent in order to create 100 more intermediate coins apiece, each of which is then spent in order to create the destination coins. The total number of coins to be spent in this example is `1 + 100 + 100^2 = 10,101` coins, or nearly 99% fewer than with the simple example.
...
#### Lazy Unrolling
From an individual's perspective, it is not strictly necessary to spend the coin that contains their payment in order to think of the payment as complete. This is because the coin that contains their payment cannot be modified. Because of this, wallets can optionally choose to display a payment that is contained within an intermediate coin as part of the spendable balance. If a user attempts to send those funds elsewhere, the wallet could spend the coin containing the payment in order to unlock its value, and then use identical spend deduplication in order to send the funds elsewhere. Two coin spends would therefore be needed, but they would both occur within the same block.
...
todo
URL included in memo of top level coin?
use DIG possibly?
...
#### Bulletins
wallet sdk call
messaging (bulletin protocol, coming soon)
subscribe to each asset ID that you own
todo

#### Centralized API

todo provide someone with your address, it replies with all of the bags that you are holding
