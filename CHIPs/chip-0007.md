...
* See [errata](#errata) regarding a missing property from this table
...
## Errata
 * The [JSON schema](../assets/chip-0007/schema.json#L91-L95) associated with this CHIP correctly states that a collection's ID is required to be a String in uuid format. However, the specific format is missing from [the specification](#properties-2) laid out in this document. Despite this omission, the format for the `id` property of a `collection` is required to be `uuid`
 * The [JSON schema](../assets/chip-0007/schema.json#L38-L47) associated with this CHIP correctly states that the `series_number` and `series_total` properties are required to be integers with a `minimum` value of `1`. However, these specific values are missing from [the specification](#properties) laid out in this document. Despite this omission, the `minimum` value for the `series_number` and `series_total` properties is `1`.
