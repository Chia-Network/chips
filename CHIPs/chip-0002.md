...
The dApp requests users' permission to connect the wallet. If the dApp has been approved, the API will return `true`. If the dApp hasn't been approved, the wallet need the user to approve mannually. If the user rejects the request, the API will throw `userRejectdRequestError`.
...
| offset    | specify which row to start retrieving from. The default value is specified by the wallet. |
...
| startHeight    | specify the block height to start querying. The default value is specified by the wallet.                                                                                                                        |
| limit          | specify the number of records to return. The default value is specified by the wallet.                                                                                                                           |
...
```tsx
signCoinSpends(params: {coinSpends: CoinSpend[]}): string
```
