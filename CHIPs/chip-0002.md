...
### accounts
To check the dApp accessible. API returns the standard pay puzzleHash if dApp is approved. Otherwise, it returns the empty list. For the single address wallets, API returns one puzzleHash. For the multiple address wallet, API returns a list of puzzleHash.
...
The dApp requests users' permission to connect the wallet. If the user rejectes the request, the API will throw `userRejectdRequestError`.
...
> If the delta is positive, the extra spend bundle should offer more amount. If the value is negative, some amount is left, and the extra spend bundle can use it.
...
### signTx
This is a lower-level API that signs custom coin spends. The signed transactions will be tracked by the wallet.The coins in the transaction will be locked.
