...
![Matching and Bit-Dropping](../assets/chip-0048//matching-search-and-bit-dropping.png)
...
![PoS2 System Overview](../assets/chip-0048/pos-challenge-to-proofs-4.png)
...
2. For passing plots, the challenge selects 4 ranged sets of **Proof Fragments**. Proof Fragments within these sets form the candidate pool for the chaining filter.
3. The **Chaining Filter** constructs a chain of 16 Proof Fragments by rotating through the 4 sets and keeping fragments that pass threshold hashing tests.
...
![Proof Fragment](../assets/chip-0048/bit-dropped-proof-fragment.png)
...
To counter this, PoS2 introduces the **Chaining Filter**, which dynamically builds the quality string as a concatenated sequence of 16 Proof Fragments. For a given challenge, a pre-filter first picks **4 chaining sets** from the plot, and a hashing function then tests which fragments to chain as the chain rotates through those 4 sets. If an attacker drops additional bits, each fragment in the chain multiplies the candidate space. With 16 chain elements, even a single bit drop produces a combinatorial explosion — (2^bits_dropped)^16 candidate chains — that quickly becomes computationally infeasible.

![Chaining Filter](../assets/chip-0048/chaining4.png)

A chain has 16 links, each contributing one Proof Fragment, and the links cycle through the 4 sets in a fixed rotation so every chain visits each set 4 times. Building a chain works in three stages:

1. **Start the chains.** A loose filter is applied to all 4 sets at once, keeping about 1 in 4 of their fragments. Each surviving fragment becomes the first link of its own chain.
2. **Extend each chain.** To add the next link, every fragment in the next set in the cycle is tested against the chain so far. Each fragment passes with probability 1 in S (S being the expected set size), so on average one fragment extends the chain at each step. This repeats link by link around the rotation.
3. **Tighten the last link.** The final link uses a stricter filter (a few extra bits plus a small fractional check) calibrated so that, after all 16 links, a challenge yields ~1 valid chain on average.

Under the hood, each link folds its fragment and a per-link key derived from the challenge into a running AES-based hash, and the filter at each step simply requires a fixed number of low bits of that hash to be zero. The tighter final filter also cancels a statistical bias (the "Jensen bonus") that arises because each set is reused 4 times and set sizes vary statistically.

With 16 links, an attacker dropping 1 bit can only support a tiny fraction of an honest farm, and with 2 bits dropped the attacker's supported farm collapses while also needing to perform hundreds of millions of false-positive proof recomputes. The current recommendation is 16 chain links, providing security against even a 100× GPU performance improvement over current hardware (see [Chaining False Positives Attack](#chaining-false-positives-attack)).
...
Challenges define four *ranged* sets of Proof Fragments as candidates for quality string construction. Because Proof Fragments are sorted, each range requires only one random access to locate the start, followed by a sequential read. A full challenge lookup therefore requires just **four random accesses and four sequential reads** — a dramatic improvement over PoS1's multi-table random lookups.
...
With grouped filtering, the plot format interleaves data from all plots in a group. When a group passes a challenge, all *N* plots respond using the same four random-access lookups and correspondingly larger sequential reads. HDD load thus depends on *sequential* read bandwidth rather than random access count — an orders-of-magnitude improvement.
...
| ------------- | --------------------------- | ---------- | ------------------ | -------- | ------------- |
| 5 TB          | base (512)                  | 1          | ~8.96%             | ~4.19%   | ~85 MB        |
| 5 TB          | base (512)                  | 16         | ~1.71%             | ~0.26%   | ~84 MB        |
| 5 TB          | +1 (1024)                   | 16         | ~1.28%             | ~0.13%   | ~44 MB        |
| 5 TB          | +2 (2048)                   | 16         | ~0.85%             | ~0.06%   | ~20 MB        |
| 5 TB          | +3 (4096)                   | 16         | ~0.85%             | ~0.03%   | ~9 MB         |
| 20 TB         | base (512)                  | 1          | ~26.46%            | ~16.90%  | ~343 MB       |
| 20 TB         | base (512)                  | 2          | ~15.79%            | ~8.45%   | ~343 MB       |
| 20 TB         | base (512)                  | 16         | ~4.70%             | ~1.03%   | ~333 MB       |
| 20 TB         | base (512)                  | 32         | ~3.00%             | ~0.54%   | ~347 MB       |
| 20 TB         | base (512)                  | 64         | ~1.72%             | ~0.26%   | ~337 MB       |
| 20 TB         | base (512)                  | 100        | ~1.29%             | ~0.17%   | ~335 MB       |
| 20 TB         | base (512)                  | 1000       | ~0.93%             | ~0.02%   | ~303 MB       |
| 100 TB        | base (512)                  | 32         | ~7.28%             | ~2.64%   | ~1.71 GB      |
| 100 TB        | base (512)                  | 64         | ~4.29%             | ~1.30%   | ~1.67 GB      |
| 100 TB        | base (512)                  | 100        | ~3.88%             | ~0.85%   | ~1.71 GB      |
| 100 TB        | base (512)                  | 1000       | ~0.93%             | ~0.09%   | ~1.72 GB      |
...
For each challenge, the attacker must validate every Proof Fragment in all 4 chaining sets, since any fragment can start a potential chain. Subsequent links only need their chaining candidates validated — but because the starters generate chains in proportion to the set sizes, most fragments in the following sets will be candidates.

This is why every Proof Fragment can start a chain: otherwise an attacker could selectively solve only the subset that forms Quality Strings.
...
|            | **Total: 61,504** | **Total: 3,560,376** | **Total: 536,854,656** | **Total: 23,456,248,037,632** |


Chaining hash rates (tuned custom AES hashing):

| Pi5 Chain Hashes/ms<br>(single thread) | 5090 Chain Hashes/ms | 5090 leverage factor |
| ------------------- | --------------- | --- |
| 18,000 | 10,000,000          | x 556 |



| Compute Model | Bits Dropped | Compression | \# Chain Links | Chain Hashes       | ms per chain (avg) | Effective Plot Filter | Supported Farm Size (eTiB) | TiB gained through compression | Overall W/TB | Overall Energy vs Honest Farmer | Hardware Cost vs Honest Farmer |
| ------------- | ------------ | ----------- | -------------- | ------------------ | ------------------ | --------------------- | -------------------------- | ------------------------------ | ------------ | ------------------------------- | ----------------------------- |
| Pi 5          | 0 (honest)   | \-          | 16             | 61504              | 3.31               | 512                   | 1303                | \-                             | 0.550        | \-                              | \-                            |
| 5090          | 1            | 3.40%       | 16             | 536,854,657        | 54.09              | 512                   | 79.7                      | 2.71                           | 5.548        | 1008.73%                        | 347.44%                       |
| 5090          | 1            | 3.40%       | 16             | 536,854,657        | 54.09              | 8192                  | 1,276                   | 43.3                          | 0.845        | 153.61%                         | 112.28%                       |
| 5090          | 2            | 6.79%       | 16             | 23,456,248,037,634 | 2,363,129      | \-                    | \-                         | \-                             |              |                                 |                               |
| 5090          | 0.5          | 1.70%       | 16             | 3,560,376          | 0.36               | 512                   | 12,023                  | 204                         | 0.574        | 104.35%                         | **99.97%**                    |
| 5090          | 0.5          | 1.70%       | 16             | 3,560,376          | 0.36               | 8192                  | 192,364                 | 3,265                       | 0.543        | **98.68%**                      | **98.41%**                    |
| 5090          | 0.83         | 2.82%       | 16             | 93,113,397         | 9.38               | 8192                  | 7,355                   | 207                         | 0.589        | 107.07%                         | **99.90%**                    |
| Pi 5          | 0 (honest)   | \-          | 32             | 127,040            | 6.83               | 512                   | 631                | \-                             |              |                                 |                               |
| 5090          | 0.5          | 1.70%       | 32             | 916,476,288        | 92.33              | 8192                  | 747                     | 12.7                          | 1.076        | 195.62%                         | 125.06%                       |
| 5090          | 0.25         | 0.85%       | 32             | 6,559,985          | 0.66               | 8192                  | 104,404                 | 886                         | 0.549        | **99.85%**                      | **99.34%**                    |
| 5090          | 0.34         | 1.15%       | 32             | 36,740,209         | 3.70               | 8192                  | 18,641                  | 215                         | 0.565        | 102.75%                         | **99.92%**                    |

#### Key Findings

An attacker is incentivized to plot at maximum strength to leverage the largest Effective Plot Filter (8192), minimizing per-challenge chaining work.

- **2 bits dropped:** Computationally infeasible — 23.5 trillion chain hashes per challenge.
- **1 bit dropped:** Both energy and hardware cost exceed honest farming (153% energy, 112% cost at filter 8192).
- **Fractional bit drops (0.5–0.83 bits):** By interleaving full and 1-bit-dropped Proof Fragments, an attacker at filter 8192 can achieve marginal savings — at best **1.32% energy** and **1.59% hardware cost** (0.5 bits, 16-chain). These are the bolded values under 100% in the table above.

The best-case attacker advantage with 16 chain links is ~1.6% savings on a 1.7% compression — meaning GPU overhead consumes nearly all of the storage benefit.

#### Effect of Increasing Chain Size to 32

Doubling the chain size forces the attacker to halve their bit drop to maintain the same hash budget, reducing the attacker's best-case savings from ~1.6% to ~0.66%. However, this comes at significant cost to the honest farmer:

- Harvesting work doubles (supported farm size halves).
- Solve time increases at every strength level, reducing the highest solvable strength by one level.
- Rental attack resistance is unaffected (plotting time does not change).

| Plot Strength | 16-chain Pi5 Solve Time | 32-chain Pi5 Solve Time | % extra time (32 vs 16) |
| ------------- | ----------------------- | ----------------------- | ----------------------- |
| base          | 4.3s                    | 4.9s                    | 14.0%                   |
| +1            | 4.5s                    | 5.2s                    | 15.6%                   |
| +2            | 4.9s                    | 6.0s                    | 22.4%                   |
...
The solve time overhead grows with strength — from 14% at base to 77% at +6 — because the additional Proof Fragments that form the chain (and Quality String) require increased matching work at higher strengths.

#### Trade-off Summary: 16-chain vs 32-chain

| Chain Size | Max Pi5 Strength (under 8s) | Base Strength Pi5 Farm Size | Max Compression | Likely Viable Compression | Best-case Attacker Savings | Other Notes |
| ---------- | --------------------------- | --------------------------- | --------------- | ------------------------- | -------------------------- | ----------- |
| 16         | +4                          | 1.3 PiB                    | 2.82%           | 1–2%                      | Up to 1.6%                 |             |
| 32         | +3                          | 630 TiB                    | 1.15%           | <0.65%                    | Up to 0.66%                | Doubles proof size on chain and validation time |

#### Distribution of Proofs Found per Challenge

Longer chains shift the distribution: more challenges produce zero proofs, with slightly heavier tails.

| Proofs Found | 16-chain distribution | 32-chain distribution |
| ------------ | -------------------- | --------------------- |
| 0            | 40.6%                | 44.4%                 |
| 1            | 33.0%                | 29.1%                 |
| 2            | 16.8%                | 15.4%                 |
...
| 10           | 0%                   | 0.006%                |
| 11           | 0%                   | 0.002%                |
| **Std dev (proofs per challenge)** | **1.10** | **1.20** |

The 32-chain distribution has 9% higher standard deviation (1.20 vs 1.10), producing more zero-proof challenges (44.4% vs 40.6%) and heavier tails (3+ proofs).

#### Practical Considerations

The analysis above uses conservative assumptions that favor the attacker:

- HDD power: 0.55 W/TB (larger or sleeping HDDs used by honest farmers would be lower, widening their advantage).
- GPU cost: $2,000 for a 5090 (current market pricing is substantially higher).
- Chain hash times: theoretical best-case for the GPU (a real implementation would incur additional overhead from memory management, scheduling, and I/O).

#### Recommendation

We recommend keeping the chain size at **16 links**. The best-case attacker advantage (~1.6% savings) is negligible in practice and likely non-existent given the conservative assumptions above. Doubling the chain size to 32 would halve honest farmer capacity and increase harvesting energy for only a marginal improvement in compression resistance (from ~1.6% to ~0.66% attacker savings). Farmers seeking additional energy savings are better served by plotting at higher strengths and lowering their W per TB usage (e.g. sleeping their drives or choosing more efficient storage). An attacker can also sleep their drives at higher plot strengths, but this shifts a larger share of their total power budget to the GPU, and further reduces their potential to bit drop. The only suitable case for an attacker is on high W/TB hardware, which does not provide a competitive advantage. 
...
![Collected Xs Attack](../assets/chip-0048/collected-xs-attack.png)
...
#### Small-Range Bias (Jensen Effect) and the Use of 4 Sets

Proof Fragment counts within a challenge range follow a Poisson distribution, so smaller ranges have proportionally larger relative variance (the relative spread scales as 1/√mean). Because the chaining filter reuses each set several times and the number of valid chains scales with a *product* of the set sizes, the expected output is a convex function of those sizes. By Jensen's inequality, random variation in set sizes inflates the expected number of valid chains above what the average size alone would predict — a "Jensen bonus." Left unaddressed, this bias rewards challenges (and plots) that happen to draw larger-than-average sets, increasing variance and giving an attacker a small statistical edge from favoring high-variance ranges.

We mitigate this in two ways. First, each challenge draws from **4 separate Proof Fragment sets** rather than one large range. Spreading the chain across 4 independent sets averages out per-set variance and shrinks the relative spread that drives the Jensen bonus. Second, the tightened final-link filter applies a small fractional correction that cancels the residual bias, keeping expected output at ~1 valid chain per challenge regardless of set-size variation.
