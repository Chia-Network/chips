...
Resistance to rental attacks is over 100× higher than in the original PoS, with scheduled adjustments to the plot ID filter, the proof fragment scan filter, and the minimum plot strength able to raise that bar over time. See CHIP-49 for more info.
...
- **Plot Strength**: is a tunable parameter that increases plot construction time, and resistance to grinding attacks.
...
The parameterization of table security also enables scheduling for stronger security over time (see scheduled plot strength).
...
2. **Plot Strength** – defines a minimum strength for valid plots, enabling expiry of older or less secure plots.
...
| **Plot expiry**   | No                                          | Yes, plots below threshold expire   | Strength-based plot expiry reduces the Plot Stability Index, but introduces a predictable and manageable lifecycle. Scheduled expirations do not affect harvester performance, and farmers can replot at any point prior to expiry.  
