CHIP Number   | 0046
:-------------|:----
Title         | Handling Dishonest NFT Offers
Description   | 
...
## Motivation
Ecosystem participants can actively shape a favorable environment for honest traders and NFT project creators while decreasing the effectiveness of dishonest traders. This proposal will help in smoothing out a couple of edge cases where the full vision of "royalties guaranteed on-chain" has not yet been realized. While increased user adoption on the Chia blockchain is desirable, this would help ensure that fewer of the new entrants arrive with the intention of scamming.
...
There are no direct impacts to backward compatibility by treating some unusual offers as suspect. However, ecosystem participants following this proposal should be cautious in making changes that could have side effects. Wallets which lacked a formalized understanding of the relationship between `trade_prices_list` and the total offer price, or recognized a different formalized understanding than what is proposed here, could have difficulty trading in the ecosystem without making updates.

## Rationale
This proposal is meant to guide users and ecosystem builders on things they should be cautious about and might require mitigation via software or behavioral modifications. A change to the default NFT transfer program and settlement layer puzzles was also considered, as was aggressive mempool validation to look for propagated dishonest offers.
...
Because the commonly available user interfaces for making multi-NFT offers don't request pricing for each NFT, the trade prices list in this case at best seems to be a guess by the client. Wallets SHOULD consider gathering user input on pricing separately for each NFT or consider limiting multi-NFT offers where royalties are in play.
...
Standardizing on trade prices list being parts of a whole that are then mirrored by the total price of an offer is REQUIRED to deliver a user experience buyers can trust.
...
## Security
From the perspective of CNI, this has been identified as explicitly NOT a security issue as neither the chain's integrity nor consensus is negatively impacted. From the perspective of NFT project owners who feel they are owed perpetual royalties from their collections, having a way to bypass these payments is considered a security issue, and the ones I talk to welcome a set of community mitigations to the issues.
