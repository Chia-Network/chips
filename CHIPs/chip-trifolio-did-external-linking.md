...
Verifying DID links to external identities can be accomplished in either 1-directional or bi-directional verifiable links.

#### 1-Directional Linking
An example of 1-directional linking would be having a link from the DID on-chain to an external identity. This can be verified in a single direction (verifying the DID on chain is associated with the external id), but not in the reverse direction (verifying the external id is owned by the same owner as the DID). Likewise, 1-directional linking can exist in the inverse case- where an external identity account owner can verify linking to a DID (using some verification mechanism from the external service), but the link may not be verifiable from the DID on chain.
...
* **type [String]**: Type of identity to verify, e.g. “facebook”
* **did [Hex String (Prefixed with 0x)]**: Hex string of the DID on-chain, e.g. “0xdeadbeef”
* **eidHash [Hex String (Prefixed with 0x)]**: SHA-256 hash of the external id, in hex, e.g. “0xdeadbeef”
...
_Example_: ```GET https://sample.authority/dids/verify?type=facebook&did=0xdeadbeef&eidHash=0xdeadbeef&direction=2&data=0xdeadbeef```
...
* **h [Hex]**: Hex id of the DID on-chain, e.g. “0xdeadbeef”
* **t [String]**: Type of identity to verify, e.g. “facebook”
...
```
{
    "eid": [
        {
            "h": "0xdeadbeef", // hash of the external id
            "a": "Sample Authority", // authority name
            "t": "facebook", // external id type
            "d": 0, // direction of linking
            "u": "https://sample.authority/dids", // URL to verification authority
            "e": "0xdeadbeef" // optional extra data used in verifying the link
        }
    ]
}
```
...
## Test Cases
### Examples
***1-Directional Link Verification Example***

1. Chia user Alice owns a DID on Chia Network identified by 2cf24dba5fb0a. 
2. Alice decides to link their Facebook account identified by 123456 to this DID using a 1-directional link with an authority called Sample Authority. 
3. Alice visits the Sample Authority for instructions on how to 1-directionally link their DID with their Facebook account
...
8. Any user may contact the Sample Authority verification API and verify that Alice’s DID 2cf24dba5fb0a has a verified 1-directional external id link to the Facebook account ID SHA-256 hash 8d969eef6ecad3c29a3a629280e686cf0c3f5d5a86aff3ca12020c923adc6c92 (SHA-256 hash of id 123456)
9. It is up to the Sample Authority to confirm the legitimacy of the 1-directional link between DID and Facebook account in the case of updates to the DID metadata

***Bi-Directional Link Verification Example***

1. Chia user Alice owns a DID on Chia Network identified by 2cf24dba5fb0a.
2. Alice decides to link their Facebook account to this DID using a 1-directional link with an authority called Sample Authority. 
