...
Arguments: The cost of the condition (specified in ten-thousands) is required as the first argument, further arguments are unspecified. The reason to scale the cost by `10 000` is to make the argument smaller. For example, a `cost` of `100` in this condition would equate to an actual cost of 1 million (`1 000 000`). The `cost` argument is two bytes, with a maxiumum size of `65 535` (an actual cost of `655 350 000`)
