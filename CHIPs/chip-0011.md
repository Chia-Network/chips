...
Arguments: A `base` value, an `exponent` (may not be negative), and a `modulus` (may not be 0)
...
Arguments: A `numerator` and a `denominator`

Usage: `(% numerator denominator)`

CLVM Cost: `998` base, `4` per byte (sum of both operands)
