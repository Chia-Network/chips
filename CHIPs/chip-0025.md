...
Functionality: Creates a `message` targeted for a specific coin. The target `coin_id` must be provided as well as the message. If the coin with the target `coin_id` is not spent in the same block where this condition is used, the condition will pass. 
