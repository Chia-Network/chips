...
This CHIP introduces two new message conditions that will make it more difficult to create inadvertently insecure spends of the types mentioned here. These new conditions will require a soft fork, which will occur at block `5 650 000` (five million, six hundred and fifty thousand).
...
We renamed the fork to `soft fork 4` in [PR #17799](https://github.com/Chia-Network/chia-blockchain/pull/17799). The previous name, `soft fork 3`, had been planned in a different CHIP that was withdrawn. We renamed this fork in order to avoid confusion between the two.
