...
The streaming identifier is defined as the coin name of the initial streaming coin. This identifier is a bech32m-encoded string with the prefix `s` (for mainnet) or `ts` (for testnet). For example, a valid testnet stream ID might be:

```
ts10f73rwr50l7q3r6nwapwk9ttptk26s4he0wmdfq4247jshnmqfgqslthzu
```
