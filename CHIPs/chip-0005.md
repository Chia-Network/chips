...
Created       | 2022-06-05
Requires      | 0002, 0004, [Singleton Standard](https://chialisp.com/docs/puzzles/singletons "Chia's Singleton Standard (pre-CHIP)"), [Offer Standard](https://chialisp.com/docs/puzzles/offers "Chia's Offer Standard (pre-CHIP)")
Replaces      | None
Superseded-By | None
...
* **Metadata URI list:** A list of URIs that map to the NFT's metadata
  ...
  * There will be multiple off-chain metadata formats, each of which will have its own CHIP. These CHIPs shall specify a list of required, recommended, and/or optional fields
  * In order for an NFT to conform to a specific off-chain metadata CHIP, it must specify `"format": "CHIP-XXXX"` within the metadata, where `XXXX` shall be the corresponding CHIP number
  ...
  * **Royalty structure:** This must be created upon minting, with the following options
    ...
    * The royalty recipient must be either an XCH address or a DID
    * The royalty recipient may be a singleton. It may be a coin that further distributes the royalty to multiple destinations
  * **Edition information:**
...
The following wallet RPC commands are optional for compliance with the NFT1 standard:
