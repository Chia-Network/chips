...
Author       | [Arvid Norberg](https://github.com/arvidn), [Richard Kiss](https://github.com/richardkiss)
...
A plot's group ID is determined by hashing its strength, public key, and either its pool contract puzzle hash or its pool public key:
...
When declaring meta groups, farmers should keep in mind that with this algorithm, plots with contiguous meta group IDs will pass adjacent signage points. For example, let's say there are three plots in `Group A`, which are assigned meta groups `0`, `1`, and `2`. As the diagram shows, there is a sliding window of 16 signage points across the meta indexes. At signage point `X`, if the plot assigned to meta group `0` passes the filter, then at signage point `X+1` the plot assigned to meta group `1` will pass, and at signage point `X+2` the plot assigned to meta group `2` will pass.

With this setup, a harvester could power up the disk containing these plots four signage points before `X`, and then power the drive down after signage point `X+2`.

Alternatively, if the meta groups for these two plots were at least 16 index values apart, then they wouldn't pass the filter within the same 16 sequential signage points.
...
The original CLVM cost of calculating the generator was `12'000 * len(blob)`. The new cost is a blended formula based on [interned tree structure](https://docs.chia.net/chia-blockchain/resources/generator-identity/technical-specification/#what-is-interning).

The complete breakdown of these updates can be found in [our documentation](https://docs.chia.net/chia-blockchain/resources/generator-identity/).
...
* [Generator identity](https://docs.chia.net/chia-blockchain/resources/generator-identity/) analysis
