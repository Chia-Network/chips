...
We will future-proof the new format by introducing two new parameters with periodic modifications: the plot ID filter, and the plot strength.  
...
The strength adjustments will continue until the strength reaches 255. The plot time and grinding resistance will both approach 2x the previous level as the strength increases.

The following table shows the block height and approximate month and year when the minimum plot strength will be adjusted.

| Block height | Month/Year (app.) | Min. strength |
| :----------- | :---------------- | :------------ |
...
These adjustments will continue until the minimum strength level reaches 255. As with the plot ID filter and the proof fragment scan filter, we can delay these adjustments with a future soft fork.
